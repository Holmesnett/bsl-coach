...
| R-010 | Ledger misclassification: a BSL trade goes unregistered, or a registered symbol is traded for non-BSL reasons in the same window | Medium | High | Registration is one flag at sizing (near-zero friction); `bsl-ledger` review list surfaces every unmatched/ambiguous fill for human review; manual tag/untag corrects | Open — Sprint 003 |
| R-011 | IBKR `realized_pnl` per-fill semantics (commission netting, partial-fill attribution) may not mean what we assume | Medium | High | Ground-truth gate: the real NOW fills must reproduce +$458.61 before the ledger is trusted; irreproducibility stops the sprint and gets documented, never papered over | Open — Sprint 003 gate |
