...
| Q-007 | Ledger sprint design: exact trade-tagging mechanics off `get_account_trades` | Architect | Sprint 003 design | **Answered 2026-07-01** | Registry-based tagging (D-018), snapshot ingest + realized_pnl ground-truth gate (D-020), ET bucketing (D-021). Spec in sprint 003 |
...
| Q-010 | PM Planning PDF retention: curated fixture set in `references/client-docs/` (Dave files daily, committed to git) + auto-archive to `data/pdf_archive/` (gitignored) once the parser migrates into `src/` | Dave (filing) / parser sprint (auto-archive) | Parser-migration sprint | Agreed 2026-07-01 | Dave starts filing dailies now; archive feature specced in the parser sprint |
| Q-011 | Daily trade journaling: `journal/YYYY-MM-DD.md` files, quantitative half auto-generated from the Sprint 003 ledger (episodes, P&L, budget state), qualitative prompts filled by Dave. Sprint 004 candidate — decide scope after Sprint 003 lands | Dave + Architect | Sprint 004 planning | Open | Raised by Dave 2026-07-01 evening; do not add to Sprint 003 (R-007) |
